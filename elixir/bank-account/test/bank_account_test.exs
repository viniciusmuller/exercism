defmodule BankAccountTest do
  ...
  @tag :pending
  test "an account can't be closed twice", %{account: account} do
    BankAccount.close_bank(account)
    assert BankAccount.close_bank(account) == {:error, :account_closed}
  end

  @tag :pending
  test "a user can't overdraft his account", %{account: account} do
    assert BankAccount.update(account, -1) == {:error, :overdraft}
    BankAccount.update(account, 10)
    BankAccount.update(account, -5)
    assert BankAccount.update(account, -6) == {:error, :overdraft}
  end
end
